...
GLfloat speed5 = 0.1f;
GLint autoViewChangeTime = 7000;

void rainf();
void night(int value);
void day(int value);
void rainDay(int value);
//update----------------------------------------------only----------------------
...
void day()
{
	...
	glutSwapBuffers();
	//rainf();
	glutTimerFunc(autoViewChangeTime, rainDay, 0);
...
void night()
{
	...
	glutTimerFunc(autoViewChangeTime, day, 0);
}
...
void rainDay()
{
	...
	for (GLfloat i = 1; i>-1; i -= .3)
	{
		for (GLfloat j = 1; j>-1; j -= .3)
		{
			glVertex2f(j, i);
			glVertex2f(j + .1, i + .2);
		}
	...
	glutSwapBuffers();
	//rainf();
	glutTimerFunc(autoViewChangeTime, night, 0);
...
void handleKeypress(unsigned char key, int x, int y) {
	...
	case 'd':
		glutDisplayFunc(day);
		glutPostRedisplay();
		PlaySound(TEXT("Traffic.wav"), NULL, SND_ASYNC | SND_FILENAME);  //Me off
		break;
	case 'n':
		glutDisplayFunc(night);
		glutPostRedisplay();
		PlaySound(TEXT("NightSounds.wav"), NULL, SND_ASYNC | SND_FILENAME);  //Me off
		break;
...
void night(int value)
{
	...
	//PlaySound(TEXT("NightSounds.wav"), NULL, SND_ASYNC | SND_FILENAME);
}
...
int main(int argc, char** argv) {
	...
	//glutDisplayFunc(day);
	PlaySound(TEXT("Traffic.wav"), NULL, SND_ASYNC | SND_FILENAME);
	glutTimerFunc(100, day, 0);//autoViewChangeTime
	//glutDisplayFunc(rainf);
